...
RUN apt-get update \
  ...
  && curl -sLO https://dl.b-data.ch/vsix/REditorSupport.r-2.6.0.vsix \
  && code-server --extensions-dir ${CODE_BUILTIN_EXTENSIONS_DIR} --install-extension REditorSupport.r-2.6.0.vsix \
