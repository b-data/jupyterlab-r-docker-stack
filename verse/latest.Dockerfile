...
RUN wget "https://travis-bin.yihui.name/texlive-local.deb" \
  ...
  && cd /tmp \
  && curl -sLO https://dl.b-data.ch/vsix/James-Yu.latex-workshop-8.16.0.vsix \
  && code-server --extensions-dir ${CODE_BUILTIN_EXTENSIONS_DIR} --install-extension James-Yu.latex-workshop-8.16.0.vsix || true \
  ## Clean up
  && rm -rf /tmp/*
